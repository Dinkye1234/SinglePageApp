...
function getProtein(item) {
  return Number(
    item?.proximates?.["Protein (g)"] ??
      item?.proximates?.["Protein(g)"] ??
      item?.proximates?.Protein ??
      0,
  );
}
...
function renderCalculationRow(food) {
  ...
  const protPct = energy
    ? (((proteinAmount * 4) / energy) * 100).toFixed(2)
    : "0.00";
  const fatPct = energy
    ? (((fatAmount * 9) / energy) * 100).toFixed(2)
    : "0.00";
  const carbPct = energy
    ? (((carbAmount * 4) / energy) * 100).toFixed(2)
    : "0.00";
...
export function recalculate(code, baseProtein, baseFat, baseCarb) {
  const amount =
    parseFloat(document.getElementById(`amount-${code}`)?.value) || 0;
...
export function updateTotals() {
  ...
  const totalProtPct = totalEnergy
    ? (((totalProtein * 4) / totalEnergy) * 100).toFixed(2)
    : "0.00";
  const totalFatPct = totalEnergy
    ? (((totalFat * 9) / totalEnergy) * 100).toFixed(2)
    : "0.00";
  const totalCarbPct = totalEnergy
    ? (((totalCarb * 4) / totalEnergy) * 100).toFixed(2)
    : "0.00";
...
export function refreshCalculationTable(nutritionData) {
  ...
  const selectedFoods = checkedCodes
    .map((code) => findFoodByCode(nutritionData, code))
    .filter(Boolean);
  ...
  resultBox.querySelectorAll(".remove-row-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const checkbox = document.querySelector(
        `input[name="foodcode"][value="${code}"]`,
      );
